...
def generate_credit_report(start_date, end_date, estimated_area_square, geometry_data, predicted_CO2, formatted_data):
    ...
    # Add predicted crop and biomass with wrapping
    y_position = 740
    y_position = draw_wrapped_text(p, f"Farm Geometry: {geometry_data['coordinates'][0]}", 100, y_position, max_width=400)
    for entry in predicted_CO2: 
        if entry > 0:
            p
    ...
    y_position = draw_wrapped_text(p, f"Predicted average carbon stock for the selected time period (kg): {calculate_positive_average(average_CO2)}", 100, y_position - 20, max_width=400)
    
    y_position = draw_wrapped_text(p, f"Estimate area square (km2): {estimated_area_square}", 100, y_position - 20, max_width=400)
    y_position = draw_wrapped_text(p, f"Estimate area square (ha): {estimated_area_square*100}", 100, y_position - 20, max_width=400)
    ...
    if estimated_area_square*100 < 0.2: 
        ...
        y_position = draw_wrapped_text(p, f"If the farm is cleared of forest for 5 years and located in FullCAM area, the {estimated_area_square:.2f} km² is eligible for to earn Australian carbon credit units.", 100, y_position - 20, max_width=400)
        y_position -= 20
...
def calculate_positive_average(array):
    """
    Calculate the average of values in a numpy array that are greater than 0.
    :param array: Numpy array of numerical values
    :return: Average of positive values or None if no positive values exist
    """
    # Filter positive values
    positive_values = array[array > 0]

    # Calculate average if positive values exist
    if positive_values.size > 0:
        return np.mean(positive_values)
    else:
        return None  # Return None if no positive values
